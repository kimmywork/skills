...
3. Pre-screen feasibility: for each major design choice, assess Feasible / Moderate / Redesigned. If Redesigned, return to `requirement-discovery`.
4. State design principles for this work.
5. Compare 2–3 approaches when choices matter; recommend one with trade-offs.
6. Map deliverable structure before tasks: components, modules, interfaces, dependencies.
7. Challenge the design: over-engineering, technology sprawl, unrealistic targets, excessive scope. Simplify if possible without losing required behavior. Assess each increment's risk level (low / medium / high).
8. Plan verifiable increments. Each must be reviewable and independently verifiable.
9. Write/update the solution and plan in the feature track folder.
