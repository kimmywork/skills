...
description: Extract durable improvements from a completed phase cycle — execution, review feedback, and fixes. Analyzes deviations from skill docs, identifies gaps, and proposes updates to existing skills or creation of new atomic skills. Use after a phase cycle's review-feedback is resolved, when recurring patterns/friction become visible, or when a phase produces repeated issues. Standalone, not bound to a specific loop. Proactive use recommended when review-feedback, repeated phase failures, or execution reveals skill gaps.
license: MIT
metadata:
  author: kenpusney
  version: "0.1.0"
...
Typically follows `review-feedback`. In `software-delivery-loop`, auto-triggers after each resolved review-feedback cycle under `full-autonomy` policy.