...
```bash
npx skills add kimmywork/skills --skill software-delivery-loop requirement-discovery solution-design implementation-execution delivery-acceptance review-feedback process-distillation
```

## Skill family

- `software-delivery-loop` — entry point and router. Routes to the correct phase, manages the review-feedback loop, and triggers process distillation.
...
- `solution-design` — principles, alternatives, architecture, deployment modes, contracts, test strategy, plan. Includes a mandatory "challenge the design" step.
- `implementation-execution` — TDD/E2E-driven vertical-slice implementation. Concrete change-control signals guard against undocumented drift.
...
- `review-feedback` — independent reviewer invoked after each phase. Inspects the current phase output together with all prior artifacts (cumulative review). Tags issues by origin phase, recommends fix-in-place or roll-back, and routes the decision.
- `process-distillation` — after each review-feedback cycle, optionally analyzes the completed phase for skill improvements. Can update existing skills or create new atomic skills. Under `full-autonomy` policy, auto-triggers and auto-approves safe improvements.
...
| Scope | Software delivery only | Agent loop design | Broad agent process | Non-trivial repo implementation | Engineering + productivity suite |
...
| Acceptance | `delivery-acceptance`; Spec Fit + Code Fit + delivery record | Verification via sub-agent | Verification and code review skills | Red/green evidence and review | Two-axis code review: Standards + Spec |
